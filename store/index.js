...
export const actions = {
  LOAD_MONSTERS_LIST (context) {
    axios.get(`${process.env.apiUrl}/monsters/?fields=slug,name,challenge_rating,type,size,hit_points,document_slug&limit=1000&ordering=slug`)
    .then(
      (response) => { context.commit('setMonstersList', response.data.results)}
    )
  },
  LOAD_SPELLS( context ) {
    axios.get(`${process.env.apiUrl}/spells/?fields=slug,name,school,dnd_class,level,components,level_int, document_slug&limit=1000`) //you will need to enable CORS to make this work
    .then(
      (response => {
