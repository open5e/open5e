module.exports = function(grunt) {
  ...
  grunt.initConfig({
        ...
        files: [
          {
              ...
              dest: 'open5e_red_theme/static/fonts/',
              filter: 'isFile'
          },
          {
              expand: true,
    ...
    sass: {
      dev: {
        ...
        files: [{
          ...
          cwd: 'sass',
          src: ['*.sass'],
          dest: 'open5e_red_theme/static/css',
          ext: '.css'
        }]
      },
      build: {
    ...
    browserify: {
      dev: {
        ...
        src: ['js/*.js'],
        dest: 'open5e_red_theme/static/js/theme.js'
      },
      build: {
        options: {
          external: ['jquery'],
    ...
    clean: {
      ...
      fonts: ["open5e_red_theme/static/fonts"]
    },
    ...
    watch: {
      ...
      sphinx: {
        files: ['open5e_red_theme/**/*', 'demo_docs/**/*.rst', 'demo_docs/**/*.py'],
  ...
  grunt.registerTask('default', ['exec:bower_update','clean:build', 'fonts', 'sass:dev','browserify:dev','exec:build_sphinx','connect','open','watch']);
  grunt.registerTask('build', ['exec:bower_update','clean:build', 'fonts', 'sass:build','browserify:build','exec:build_sphinx']);
}
